...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    html += "<tr><td>Thermocouple</td><td>%TC_TEMP% &deg;F</td><td></td><td></td></tr>";
    html += "<tr><td>Thermistor 1 (Meat)</td><td>%T1_TEMP% &deg;F</td><td>%MEAT_TEMP% &deg;F</td>";
    html += "<td>";
...
void loop() {
  ...
  if (meatTempMode != lastMode) {
    // Update mode display on line 4
    lcd.setCursor(14, 3);  // Position after "M:"
    lcd.print("      "); // Clear previous value (6 chars to prevent overflow)
    lcd.setCursor(14, 3);
